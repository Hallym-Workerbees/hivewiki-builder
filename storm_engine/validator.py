...
NUMERIC_ACCURACY_PROMPT = """다음은 원본 공지와 그로부터 생성된 위키입니다.
위키에 등장하는 다음 항목들을 원본과 대조하세요:
- 날짜, 시간, 기간
- 전화번호, 이메일, 팩스, URL
- 호실, 건물명, 부서명
- 인명, 학번, 직책
...
- 형식:
[
  {{"entity": "항목명", "wiki_says": "위키 값", "source_says": "원본 값 또는 '없음'"}}
]"""
...
def _check_numeric_accuracy(
    wiki_markdown: str, source_text: str, openai_client: OpenAI
) -> list[ValidationIssue]:
    prompt = NUMERIC_ACCURACY_PROMPT.format(
        source=source_text[:SOURCE_MAX_CHARS],
...
def validate(
    ...
    issues.extend(_check_numeric_accuracy(wiki_markdown, source_text, openai_client))

    by_category: dict[str, int] = {}
    for issue in issues:
        by_category[issue.category] = by_category.get(issue.category, 0) + 1
    logger.info(
        "[VALIDATOR] passed=%s total=%s breakdown=%s",
        not issues,
        len(issues),
        by_category,
    )
